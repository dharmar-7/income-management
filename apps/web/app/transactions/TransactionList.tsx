    ...
    <div className="space-y-4">
      ...
      <div className="bg-white rounded-2xl border border-gray-200 p-4 flex flex-wrap gap-3 items-end">
        <div className="flex gap-2 flex-1 min-w-48 relative">
          <input
            ...
            className="flex-1 border border-gray-200 rounded-full px-4 py-2 pl-10 text-sm focus:outline-none focus:border-gray-400"
          />
      ...
      <div className="bg-white rounded-2xl border border-gray-200 overflow-hidden">
        {isLoading ? (
          ...
          <div className="divide-y divide-gray-100">
            {data?.data.map(tx => (
              <div key={tx.id} className="flex items-center hover:bg-gray-50 transition-colors group">
                <Link
                ...
                >
                  ...
                  <div className={`w-10 h-10 rounded-full flex items-center justify-center text-lg flex-shrink-0 ${
                    ...
                    : tx.type === 'REFUND' ? 'bg-teal-50'
                    : tx.type === 'INVESTMENT' ? 'bg-indigo-50'
                    : 'bg-rose-50'
                  }`}>
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      ...
                      {tx.type === 'INVESTMENT' && (
                        <span className="text-xs bg-indigo-50 text-indigo-600 border border-indigo-100 rounded-full px-2 py-0.5 flex-shrink-0 leading-none">
                          Investment
                        </span>
                      )}
                      {tx.type === 'REFUND' && (
                        <span className="text-xs bg-teal-50 text-teal-600 border border-teal-100 rounded-full px-2 py-0.5 flex-shrink-0 leading-none">
                          Refund
                        </span>
                      )}
                      {tx.source === 'MANUAL' && (
                        <span className="text-xs bg-violet-50 text-violet-500 border border-violet-100 rounded-full px-2 py-0.5 flex-shrink-0 leading-none">
                          Manual
                        </span>
                      )}
                      {tx.source === 'SMS' && (
                        <span className="text-xs bg-blue-50 text-blue-500 border border-blue-100 rounded-full px-2 py-0.5 flex-shrink-0 leading-none">
                          SMS
                        </span>
                      )}
                    </div>
                ...
                {tx.source === 'MANUAL' && (
                  <button
                    ...
                    className="mr-5 opacity-0 group-hover:opacity-100 transition-opacity text-gray-300 hover:text-rose-400 disabled:opacity-40 text-lg"
        ...
        <div className="flex items-center justify-between px-2">
          ...
          <div className="flex gap-2">
            <button
              ...
              disabled={page === 1}
              className="rounded-full border border-gray-200 px-4 py-2 text-sm disabled:opacity-40 hover:bg-gray-50"
            >
              Previous
            </button>
            <button
