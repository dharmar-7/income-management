...
const COLOR_MAP: Record<string, string> = {
  ...
  orange: '#ffedd5',
  green:  '#dcfce7',
  mirror: 'rgba(255,255,255,0.28)',  // liquid glass — semi-transparent
};
...
function NoteSheet({
  ...
  async function handleSave() {
    ...
    try {
      ...
      if (reminderAt && saved.id) {
        await scheduleLocalNotification(saved.id, saved.title, saved.content, new Date(reminderAt));
      } else if (saved.id) {
  ...
  async function handlePickImage() {
    if (!note?.id) {
      Alert.alert('Save first', 'Please save the note before adding images.');
      return;
    }
              ...
              <ScrollView horizontal showsHorizontalScrollIndicator={false}>
                <View style={styles.colorRow}>
                  {COLOR_KEYS.map(c => (
                    ...
                    >
                      {c === 'mirror' && (
                        /* Chrome/silver diagonal highlight */
...
export default function NotesScreen() {
  ...
  return (
    <SafeAreaView style={styles.safe}>
      {/* Vivid background blobs — BlurView on mirror cards smears these into liquid glass */}
